...
module stemouter() {
  ...
  stemdia_x = 8;//adjusted_side_x(true) + extra_layers;
...
module stem() {
  difference() {
    stemouter();
    translate([0,0,-switch_offset]) cylinder(d=3.12+inner_slop,h=2.6-.2);

  }
}

module clampify(nowashers=true) {
  washerdia=3.8+.2;
  nutdia=3.5+.2;// + .05;
  screwhead=3+.2;
  screwdia=1.9;
  ...
  difference(){
    union() {
      ...
      if(nowashers){
	extra_nut_cup = 0;//.65;
	rotational_clone() rotate([0,0,45]) translate([-tabwidth/2-extra_nut_cup,tabsep/2,0]) cube([tabwidth+extra_nut_cup,tablen,tabheight]);
      }else{
	rotational_clone() rotate([0,0,45]) translate([-tabwidth/2,tabsep/2,0]) cube([tabwidth,tablen,tabheight]);
      }
    }
    let(width=.85) {
      rotational_clone() rotate([0,0,45]) translate([-width/2,-1,-.1]) cube([width,2*tablen,tabheight+.11]);
    }

    if(nowashers){
      	extra_nut_cup = .5;
      rotational_clone() rotate([0,0,45]) translate([0,screwsep,tabheight/2]) {
